    ...
    <v-card-text>
      ...
      <div v-if="!hasEmails">
        <div class="flex justify-center items-center">
          <div class="text-base">No emails found</div>
        </div>
      </div>
      <v-table v-if="hasEmails" density="compact">
        ...
        <tbody>
          <tr v-for="(item, index) in props.freightForwarder.emails" :key="`ff-email-${index}`">
            <td>
...
const hasEmails = computed(() => {
  return props.freightForwarder.emails?.length > 0
})
...
const onSuccess = async (values: any) => {
  try {
    ...
    const body = {
      ...values,
      id: props.freightForwarder.id,
    }
    await $api.freightForwarders.addEmail(props.freightForwarder.id, body)
...
const showConfirmDelete = async (ffEmail: any) => {
  ...
  if (result) {
    try {
      ...
      const body = {
        id: ffEmail.id,
        freight_forwarder_id: props.freightForwarder.id,
      }
      await $api.freightForwarders.deleteEmail(props.freightForwarder.id, body)
