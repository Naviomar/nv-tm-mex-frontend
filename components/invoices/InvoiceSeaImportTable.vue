  ...
  <div>
    <div class="mb-4" @keyup.enter="onClickFilters">
      <div class="grid grid-cols-12 gap-2">
        ...
        <div class="col-span-2">
          <v-text-field v-model="filters.masterBl" density="compact" hide-details label="Master BL" />
        </div>
        <div class="col-span-2">
          <v-text-field v-model="filters.houseBl" density="compact" hide-details label="House BL" />
        </div>
        <div class="col-span-4">
          <ACustomerSearch v-model="filters.consignee_id" />
        </div>
        <div class="col-span-2">
          <v-text-field v-model="filters.containerNumber" density="compact" label="Container #" hide-details />
        </div>
        <div class="col-span-2">
          <v-text-field v-model="filters.bookingNum" density="compact" label="Booking number" hide-details />
        </div>
        <div class="col-span-2">
          <v-text-field v-model="filters.poNum" density="compact" label="PO number" hide-details />
        </div>
        ...
        <v-table density="compact" fixed-header height="75vh">
          ...
          <tbody>
            <tr
              ...
              :key="`reference-${index}`"
              class="dark:hover:bg-gray-700 hover:bg-slate-300"
            >
              <td>
                <v-btn
                  variant="text"
              ...
              <td>
                <div v-if="item.charges?.length <= 0 && item.sell_rate_breakdown?.length <= 0">
                  <v-chip size="small" variant="elevated" color="error">No sell concepts</v-chip>
                </div>
                <div v-if="item.charges?.length > 0" class="flex flex-col gap-1 mb-2">
                  <div class="text-xs">Local charges</div>
                  <v-chip
                    v-for="(charge, index) in item.charges"
                    :key="`charge-${index}`"
                    size="small"
                ...
                <div v-if="item.sell_rate_breakdown?.length > 0" class="flex flex-col gap-1">
                  <div class="text-xs">Sell breakdown charges</div>
                  <v-chip
                ...
                <div v-if="item.demurrage_charges?.length > 0" class="flex flex-col gap-1">
                  <div class="text-xs">Demurrage charges</div>
                  <v-chip
                    v-for="(demurrageCharge, index) in item.demurrage_charges"
                    :key="`dem-charge-${index}`"
                    size="small"
                    :color="demurrageCharge.invoice_charge ? 'primary' : 'error'"
                    variant="flat"
                  >
                    <v-icon>{{ demurrageCharge.invoice_charge ? 'mdi-link' : 'mdi-link-off' }}</v-icon>
