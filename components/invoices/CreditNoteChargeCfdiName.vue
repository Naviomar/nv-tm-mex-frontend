...
const hasOtherNames = computed(() => {
  ...
  return props.names.some((x: any) => x.charge_id === props.creditNote.charge?.id)
})

const chargeNames = computed(() => {
  return props.names.filter((x: any) => x.charge_id === props.creditNote.charge?.id)
})

const hasOtherChargeName = computed(() => {
  return props.creditNote.charge_name != null
})

const chargeName = computed(() => {
  if (props.creditNote.charge_name != null) {
    return props.creditNote.charge_name
  }
  return props.creditNote.charge?.name
})
...
const saveCfdiName = async () => {
  try {
    ...
    const response = await $api.consigneeCreditNotes.updateChargeCfdiName(body)

    props.creditNote.charge_name = JSON.parse(JSON.stringify(form.value.cfdi_name.name))
