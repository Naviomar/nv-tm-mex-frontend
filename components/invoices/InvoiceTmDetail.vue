            ...
            <div class="flex flex-wrap gap-2 md:gap-4 items-center">
              ...
              <div v-if="invoiceTm.from_deleted_invoice">
                <v-chip size="small" color="orange" variant="tonal">
                  Re-invoice from 
                  <NuxtLink 
                    :to="`/invoices/search/tm-view-${invoiceTm.from_deleted_invoice}`" 
                    target="_blank"
              ...
              <div v-if="invoiceTm.replacement_invoice_id">
                <v-chip size="small" color="blue" variant="tonal">
                  Replaced by 
                  <NuxtLink 
                    :to="`/invoices/search/tm-view-${invoiceTm.replacement_invoice_id}`" 
                    target="_blank"
        ...
        <v-card-text>
          <div v-if="isFreeFormat" class="mb-4">
            <v-alert type="info" variant="tonal" density="compact">
              <div class="font-semibold">Free format - No reference linked</div>
            </v-alert>
          </div>
              ...
              <v-card-text>
                <div class="flex items-start gap-3">
                  ...
                  <div class="flex-1">
                    ...
                    <div class="font-bold text-base">{{ invoiceTm.consignee?.name }}</div>
                  </div>
                </div>
              </v-card-text>
            </v-card>
              ...
              <v-card-text>
                <div class="flex items-start gap-3">
                  ...
                  <div class="flex-1">
                    ...
                    <div class="font-bold text-xl">{{ getCurrencyName(invoiceTm.invoice?.currency_id) }} {{ formatToCurrency(invoiceTm.invoice?.total) }}</div>
                    <div v-if="invoiceTm.invoice?.currency_id != 2" class="text-sm mt-1">
                      <span class="text-grey-darken-1">Exchange Rate:</span> {{ formatToCurrency(1 / invoiceTm.exchange_rate) }}
                    </div>
                  </div>
                </div>
              </v-card-text>
          ...
          <v-card variant="tonal" color="purple" class="mb-4">
            <v-card-text>
              <div class="flex items-center gap-2 mb-3">
                ...
                <div class="text-sm font-bold text-purple-darken-2">FISCAL INFORMATION</div>
              </div>
              <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                <div>
                  <div class="text-xs text-grey-darken-1 mb-1">RFC</div>
            ...
            <v-card-text>
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">
                  ...
                  <div class="font-medium">{{ invoiceTm.address || 'No address provided' }}</div>
                </div>
              </div>
            </v-card-text>
          </v-card>
          ...
          <div class="flex flex-col sm:flex-row justify-around gap-2 mb-2">
            <PreviewTmInvoice service="sea" :invoice="invoiceTm" />
            ...
            <AuthorizeProcessSmart
              ...
              :resource="authorizeResources.CancelInvoiceTm.resource"
          ...
          <v-card-text>
            <div class="flex gap-2">
              <v-btn
                ...
                size="small"
                variant="tonal"
                :to="`/maritime/${service.referencia.impoExpo == 'I' ? 'import' : 'export'}/view-details-${
                  service.referencia?.id
                }`"
              >
...
const getReferenceNumber = (referenciaId: number) => {
  return invoiceTm.value?.services.find((service: any) => service.referencia_id === referenciaId)?.referencia
    ?.reference_number
}
...
const getData = async () => {
  try {
    loadingStore.start()
    const response = (await $api.tmInvoices.getById(props.id)) as any
...
const onEditProformaClick = async () => {
  router.push(`/invoices/search/tm-${props.id}-edit-proforma`)
}

const onSyncClientInfoClick = async () => {
  try {
    loadingStore.start()
    await $api.tmInvoices.syncClientInfo(props.id)
...
const cancelInvoice = async () => {
  try {
    ...
    loadingStore.start()
    const { error } = (await $api.tmInvoices.cancel(props.id, cancel.value)) as any
