<template>
  <div>
    ...
    <v-dialog v-model="form.showDialog" max-width="600">
      <v-card>
        <v-card-title>
          <span class="headline">Attachments - Supplier Req. Payment {{ serviceName }}</span>
        </v-card-title>
        <v-card-text>
...
const serviceName = computed(() => props.supReqPayment?.folio || '#' + props.supReqPayment?.id)
...
const showConfirmDelete = async (item: any) => {
  ...
  if (result) {
    try {
      ...
      await $api.supplierReqPayments.removeAttachment(props.supReqPayment.id.toString(), body)
...
const onUploadClick = async () => {
  try {
    ...
    loadingStore.start()
    const response = (await $api.supplierReqPayments.uploadAttachments(props.supReqPayment.id, body)) as any
