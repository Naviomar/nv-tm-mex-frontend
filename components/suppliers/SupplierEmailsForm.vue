    ...
    <v-card-text>
      ...
      <v-table density="compact">
        ...
        <tbody>
          ...
          <tr v-for="(supEmail, index) in props.supplier.emails" :key="`ff-email-${index}`">
            <td>
...
const hasEmails = computed(() => {
  return props.supplier.emails?.length > 0
})
...
const onSuccess = async (values: any) => {
  try {
    ...
    await $api.suppliers.addEmail(props.supplier.id, body)
...
const showConfirmDelete = async (ffEmail: any) => {
  ...
  if (result) {
    try {
      ...
      const body = {
        id: ffEmail.id,
        freight_forwarder_id: props.supplier.id,
      }
      await $api.suppliers.deleteEmail(props.supplier.id, body)
