  ...
  <v-card class="mb-4">
    ...
    <v-card-text>
      ...
      <v-table density="compact">
        ...
        <tbody>
          <tr v-for="(item, index) in addresses" :key="`supplier-address-${index}`">
            <td>
...
const getSupplierAddresses = async () => {
  const response = await $api.suppliers.getAddresses(props.id.toString())
...
const onSuccess = async (values: any) => {
  const body = {
    ...values,
    supplier_id: props.id.toString(),
  }
  const response: any = await $api.suppliers.updateAddress(props.id.toString(), body)
...
const showConfirmDelete = async (id: any) => {
  ...
  if (result) {
    try {
      ...
      await $api.suppliers.deleteAddress(props.id.toString(), id)
