    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
      <v-card class="mb-4">
        ...
        <v-card-text>
          ...
          <v-card variant="tonal" color="blue" class="mb-4">
            <v-card-text>
              <div class="flex items-start gap-3">
                <v-icon size="32" color="blue-darken-2">mdi-domain</v-icon>
                <div class="flex-1">
                  <div class="text-xs text-blue-darken-1 font-semibold mb-1">{{ getPartyableType(partyInvoice.partyable_type)?.toUpperCase() || 'PARTY' }}</div>
                  <div class="font-bold text-base">{{ partyInvoice.partyable?.name }}</div>
                  <div class="text-sm mt-1">
                    <span class="text-grey-darken-1">Serie:</span> <span class="font-semibold">{{ partyInvoice.inv_serie }}</span>
                  </div>
                </div>
              </div>
            </v-card-text>
            ...
            <v-card-text>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                <div>
                  ...
                  <div class="font-semibold">{{ partyInvoice.rfc }}</div>
                </div>
                <div>
                  <div class="text-xs text-grey-darken-1 mb-1">Régimen Fiscal</div>
                  <div class="font-semibold">{{ partyInvoice.regimen_fiscal }}</div>
                </div>
                <div>
                  <div class="text-xs text-grey-darken-1 mb-1">Uso CFDI</div>
                  <div class="font-semibold">{{ partyInvoice.uso_cfdi }}</div>
                </div>
                <div>
                  <div class="text-xs text-grey-darken-1 mb-1">Método de pago</div>
                  <div class="font-semibold">{{ partyInvoice.metodo_pago }}</div>
                </div>
                <div>
                  <div class="text-xs text-grey-darken-1 mb-1">Forma de pago</div>
                  <div class="font-semibold">{{ partyInvoice.forma_pago }}</div>
                </div>
              </div>
            </v-card-text>
          </v-card>
          ...
          <div v-if="!isCancelled" class="flex flex-col sm:flex-row flex-wrap justify-start gap-2 my-4">
