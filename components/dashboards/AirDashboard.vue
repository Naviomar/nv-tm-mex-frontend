  ...
  <div>
    <div class="p-4 bg-sky-100 dark:bg-neutral-800 rounded-lg">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
        <v-card>
          ...
          <v-card-text>
            ...
            <v-icon icon="mdi-airplane" size="64" class="absolute! right-4 top-1/2 -translate-y-1/2 opacity-30 z-0" />
          </v-card-text>
        </v-card>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <v-card>
          <v-card-title class="text-overline"> Pending notification </v-card-title>
          <v-card-text>
            <div class="text-blue-darken-3 text-h3 font-weight-bold">
          ...
          <div class="px-4 text-xs">Air Import</div>
        </v-card>
        <v-card>
          <v-card-title class="text-overline"> Pending release </v-card-title>
          <v-card-text>
            <div class="text-blue-darken-3 text-h3 font-weight-bold">
        ...
        <v-card>
          <v-card-title class="text-overline"> Import CBM Summary </v-card-title>
          <v-card-text>
            <div class="text-blue-darken-3 text-h5 font-bold mb-2">
              Total CBM: {{ report.import_cbm_summary?.total_cbm ?? '0.00' }}
            </div>
            <div class="text-sm">Total M3: {{ report.import_cbm_summary?.total_m3 ?? '0.00' }}</div>
            <div class="text-sm">Total Packages: {{ report.import_cbm_summary?.total_packages ?? 0 }}</div>
            <!-- Icono decorativo -->
            <v-icon
        ...
        <v-card>
          <v-card-title class="text-overline"> Export CBM Summary </v-card-title>
          <v-card-text>
            <div class="text-blue-darken-3 text-h5 font-bold mb-2">
              Total CBM: {{ report.export_cbm_summary?.total_cbm ?? '0.00' }}
            </div>
            <div class="text-sm">Total M3: {{ report.export_cbm_summary?.total_m3 ?? '0.00' }}</div>
            <div class="text-sm">Total Packages: {{ report.export_cbm_summary?.total_packages ?? 0 }}</div>
            <!-- Icono decorativo -->
            <v-icon
              icon="mdi-cube-outline"
              size="64"
