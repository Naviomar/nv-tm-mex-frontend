<template>
  <div>
    <v-card class="mb-4">
      <v-card-title>
        <h1 class="font-bold">Request demurrages payment to freight line</h1>
      </v-card-title>
      <v-card-text>
    ...
    <v-card>
      ...
      <v-card-text>
        ...
        <v-table>
          <thead>
            <tr>
              ...
              <th>MBL</th>
              <th>Container</th>
              <th class="bg-orange-500!">Freight line demurrages</th>
              <th>Vessel</th>
              <th>Service #</th>
              <th>Customer</th>
              <th>ETD</th>
          ...
          <tbody>
            <tr v-for="(refContainer, index) in refContainers" :key="`result-bl-${index}`">
              <td>
                <div>
                  <v-checkbox
                    ...
                    hide-details
                    :disabled="!hasLineDemuarragesAvailable(refContainer)"
                  />
                </div>
              </td>
              ...
              <td class="bg-orange-500 whitespace-nowrap">
                <div v-if="!refContainer.demurrage">No demurrages initiated</div>
                <div v-if="refContainer.demurrage?.req_demurrage_container">Already requested</div>
                <div v-if="!hasLineDemuarragesAvailable(refContainer)">
                  <div>No demurrages to pay</div>
                </div>
                <div v-if="hasLineDemuarragesAvailable(refContainer)">
                  <div>
                    Pending to pay: {{ formatToCurrency(getLinePayDemurrages(refContainer)) }}
                    {{ refContainer.demurrage.line_iva ? ' incl. IVA' : '' }}
                  </div>
                </div>
              </td>
        ...
        <div v-if="hasSelected" class="py-4">
          ...
          <v-btn color="primary" size="small" @click="requestPayDemurrage">Create request payment</v-btn>
        </div>
...
const selectAll = () => {
  refContainers.value.forEach((refContainer: any) => {
    if (hasLineDemuarragesAvailable(refContainer)) {
...
const totalAmountSelected = computed(() => {
  return refContainers.value
    ...
    .reduce((acc: number, refContainer: any) => acc + getLinePayDemurrages(refContainer), 0)
})
...
const hasLineDemuarragesAvailable = (refContainer: any) => {
  if (!refContainer.demurrage) return false

  // Sum all requested amounts from req_demurrage_containers
  const totalRequested =
    refContainer.demurrage.req_demurrage_containers?.reduce(
  ...
  const totalDemurrage =
    parseFloat(refContainer.demurrage.line_cost) + (parseFloat(refContainer.demurrage.line_iva) || 0)
  const availableAmount = totalDemurrage - totalRequested

  return availableAmount > 0
}

const getLinePayDemurrages = (refContainer: any) => {
  if (!refContainer.demurrage) return 0

  // Total Demurrage Cost (including IVA)
  const totalDemurrage =
    parseFloat(refContainer.demurrage.line_cost) + (parseFloat(refContainer.demurrage.line_iva) || 0)
  ...
  return Math.max(0, totalDemurrage - totalRequested)
}
...
const searchForDemurrages = async () => {
  try {
    ...
    const response = (await $api.maritimeDemurrages.searchServicesToRequest(body)) as any
...
// reqForm.value.origin_bank to set inv_type
watch(
