...
import { ref } from 'vue'
import type { DateOrString, CatalogOption, ExportAccountStatementFilters } from '~/typings/reports/reports'
...
const filters = ref<ExportAccountStatementFilters>({
  fromDate: fromDate,
  toDate: toDate,
  voyage_id: null,
  shipper_id: null,
...
const applyFilters = async () => {
  try {
    ...
    const response = await $api.exportAccountStatement.exportExcel(queryFilters)
    ...
    link.href = url
    link.setAttribute('download', `export_account_statement_${formatDate(new Date())}.xlsx`)
