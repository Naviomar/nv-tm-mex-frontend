    ...
    <v-card elevation="0" class="report-card">
        ...
        <div class="header-content">
          <div class="header-left">
            <div class="icon-wrapper">
              <v-icon color="white" size="28">mdi-file-clock-outline</v-icon>
            </div>
            <div class="header-text">
              <h2 class="report-title">Charges Pending Invoice</h2>
              <p class="report-subtitle">Outstanding charges awaiting invoice generation</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <v-card-text class="pa-6">
        ...
        >
          ...
          This report shows all charges that are pending invoice generation.
        </v-alert>
      </v-card-text>
    </v-card>
  </div>
...
const applyFilters = async () => {
  try {
    ...
    const response = await $api.reports.getChargesPendingInvoice({
      query: queryFilters,
    })
...
.report-header { background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%); position: relative; overflow: hidden; }
