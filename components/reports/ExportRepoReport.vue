    ...
    <v-card elevation="0" class="report-card">
        ...
        <div class="header-content">
          <div class="header-left">
            ...
            <div class="header-text">
              <h2 class="report-title">General Export Report</h2>
              <p class="report-subtitle">General export report with financial data and TEUS calculation</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <v-card-text class="pa-6">
        ...
        >
          ...
          This report generates an Excel file with export reference data including financial calculations, TEUS, and container information from legacy and/or new system databases.
        </v-alert>
      </v-card-text>
    </v-card>
  </div>
...
import { ref } from 'vue'
import type { DateOrString, CatalogOption, ExportRepoFilters } from '~/typings/reports/reports'
...
// (terrTipo): RAIL, TRUCK, RAIL_TRUCK. Value 'RAILTRUCK' (no underscore) is the
...
const filters = ref<ExportRepoFilters>({
  fromDate: fromDate,
  toDate: toDate,
  voyage_id: null,
  consignee_id: null,
...
const applyFilters = async () => {
  try {
    ...
    const response = await $api.exportRepo.exportExcel(queryFilters)
    ...
    link.href = url
    link.setAttribute('download', `export_repo_general_${formatDate(new Date())}.xlsx`)
