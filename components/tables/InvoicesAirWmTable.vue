          ...
          <tr v-for="(invoiceWm, index) in wmInvoices.data" :key="`invoice-${index}`" :class="rowClass(invoiceWm)">
            <td>
              <div class="flex flex-col items-center gap-2">
                ...
                <PreviewWmInvoice service="air" :invoice="invoiceWm" size="small" />
              </div>
            </td>
            <td>
            ...
            <td>
              <v-chip size="small" color="primary"> {{ invoiceWm.is_proforma ? 'Profoma' : 'Invoice' }} </v-chip>
              <div v-if="invoiceWm.from_deleted_invoice != null" class="mt-1">
                <v-chip size="small" color="orange" variant="tonal">
                  Re-invoice from 
                  <NuxtLink 
                    :to="`/invoices/search/wm-air-view-${invoiceWm.from_deleted_invoice}`" 
                    target="_blank"
              ...
              <div v-if="invoiceWm.replacement_invoice_id != null" class="mt-1">
                <v-chip size="small" color="blue" variant="tonal">
                  Replaced by 
                  <NuxtLink 
                    :to="`/invoices/search/wm-air-view-${invoiceWm.replacement_invoice_id}`" 
                    target="_blank"
            ...
            <td>
              <div class="grid grid-cols-1 gap-1 overflow-hidden">
                ...
                >
                  {{ charge.charge?.name }} - {{ charge.amount }} {{ getCurrencyName(charge.currency_id) }}
                </div>
              </div>
            </td>
            <td>
...
const {
  ...
  storageKey: 'invoices-air-wm-filters',
...
const backUrl = computed(() => getFilteredUrl('/invoices/search/air-import-wm'))
...
const exportToExcel = async () => {
  try {
    ...
    const response = (await $api.wmAirInvoices.exportInvoicesToExcel({
      query: {
    ...
    link.download = `export-invoices-air-wm-${Date.now()}.xlsx`
    link.click()
  } catch (e) {
    console.error(e)
  } finally {
...
const getData = async () => {
  try {
    ...
    const response = (await $api.wmInvoices.getAirPaged({
      query: {
