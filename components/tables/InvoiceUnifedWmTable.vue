    ...
    <div>
        <div @keyup.enter="onClickFilters">
            ...
            <div class="grid grid-cols-5 gap-5">
                ...
                <div>
                    <v-autocomplete v-model="filters.serviceType" :items="[
                        { id: 'all', value: 'All' },
                        { id: 'sea', value: 'Maritime' },
                        { id: 'air', value: 'Air' },
                    ]" item-title="value" item-value="id" density="compact" label="Service type"
                ...
                <div>
                    <v-text-field v-model="filters.referencia" density="compact" label="Add reference #"
                        ...
                        <template #append-inner>
                            <v-btn v-if="filters.referencia" icon="mdi-plus" size="x-small" variant="text"
                                color="primary" @click="addReferencia" title="Add reference to filter" />
                        </template>
                    </v-text-field>
                </div>
                <div>
        ...
        <div>
            ...
            <v-table density="compact" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(invoiceWm, index) in wmInvoices.data" :key="`invoice-${index}`"
                        :class="rowClass(invoiceWm)">

                        <td>
                            <div class="flex flex-col items-center gap-2">
                                <ViewButton :item="invoiceWm" @click="viewInvoice(invoiceWm)" />
                                <PreviewTmInvoice :service="invoiceWm.service_type || 'sea'" :invoice="invoiceWm"
                                    size="small" />
                            </div>
                        </td>
                        <td>
                        ...
                        <td>
                            ...
                            <div v-if="invoiceWm.is_free_format == 0">
                                <v-chip v-for="(service, index) in invoiceWm.services" :key="`service-${index}`"
                                    size="small" color="primary" @click="viewService(service)">
                        ...
                        <td>
                            ...
                            <div v-if="invoiceWm.from_deleted_invoice != null">
                                <v-chip size="small" color="red">Linked to deleted invoice #{{
                                    invoiceWm.from_deleted_invoice
                                    }}</v-chip>
                            </div>
                        </td>
                        <td>
                            <div v-if="invoiceWm.invoice?.consignee_credit_notes.length > 0"
                                ...
                                <v-chip v-for="(creditNote, index) in invoiceWm.invoice?.consignee_credit_notes"
                                    size="small" color="red" :key="`ccn-${index}`" @click="viewCreditNote(creditNote)">
                                    <div class="flex gap-1"><v-icon>mdi-eye-outline</v-icon> CN #{{ creditNote.id }}
                                    </div>
                                </v-chip>
                            </div>
                        </td>
                        ...
                        <td class="whitespace-nowrap">
                            <div v-for="(totalCurrency, index) in getCurrenciesTotal(invoiceWm)"
                                :key="`currency-total-${index}`">
                                {{ totalCurrency }}
                            </div>
                        </td>
