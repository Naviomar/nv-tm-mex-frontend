            ...
            >
              <td>
                <div class="flex">
                  ...
                  ></v-btn>
                  <v-btn
                    v-if="hasPermission('air-export-references-view')"
                    variant="text"
              ...
              <td>
                <TrashButton :item="item" serviceType="air-export" @click="confirmDeletion" />
              </td>
            </tr>
          </tbody>
        </v-table>
...
const confirmDeletion = async (item: any) => {
  try {
    loadingStore.start()
    await $api.airExport.deleteReference(item.id.toString())
