      ...
      <v-card-title>
        <div class="flex gap-4">
          <div class="flex items-center">
            ...
            <div class="ml-2 font-bold">Customer notes</div>
          </div>
          <div>
...
import { schemaNotes } from '~/forms/consigneeForm'
...
const onSuccess = async (values: any) => {
  try {
    ...
    const response = await $api.consignees.addCustomerNote(props.customer.id.toString(), values)
...
const showConfirmDelete = async (customer: any) => {
  ...
  if (result) {
    try {
      loadingStore.start()
      const response = await $api.consignees.deleteNote(customer.id)
      snackbar.add({ type: 'success', text: 'Note deleted' })
      notes.value = notes.value.filter((note: any) => note.id !== customer.id)
    } catch (e) {
      console.error(e)
    } finally {
      setTimeout(() => {
...
const getReferencesNotes = async () => {
  const response = await $api.lines.getLineNotes(props.customer.id.toString())
