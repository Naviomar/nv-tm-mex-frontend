    ...
    <v-card color="blue-lighten-4" variant="flat" class="mb-4" density="compact">
      <v-card-title>
        <div class="flex justify-between items-center text-base">
          <div>
            ...
            <div v-if="hasCustomer"><v-btn size="x-small" @click="goToCustomers">Go to customer config</v-btn></div>
          </div>
      ...
      <v-card-text v-if="showInfo">
        <v-table density="compact">
          ...
          <tbody>
            <tr v-if="customer.banks.length === 0">
              <td colspan="3" class="text-center">No bank accounts found</td>
            </tr>
            <tr v-for="(bank, index) in customer.banks" :key="`customer-${index}`">
              <td>
...
const hasCustomer = computed(() => !!customer.value.id)
...
const goToCustomers = () => {
  navigateTo('/configuration/customers/edit-' + props.id, {
    open: {
      target: '_blank',
    },
  })
...
const getCustomer = async () => {
  try {
    loadingStore.start()
    const response = await $api.consignees.getById(props.id.toString())
...
// watch props.id if null clear customer
watch(
  () => props.id,
  (id) => {
    if (!id) {
      customer.value = { banks: [] }
