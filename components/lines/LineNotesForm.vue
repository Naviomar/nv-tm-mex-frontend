      ...
      <v-card-title>
        <div class="flex gap-4">
          <div class="flex items-center">
            ...
            <div class="ml-2 font-bold">Freight Line notes</div>
          </div>
          <div>
...
import { schemaNotes } from '~~/forms/lineForm'
...
const onSuccess = async (values: any) => {
  try {
    ...
    const response = await $api.lines.addLineNote(props.line.id.toString(), values)
...
const getReferencesNotes = async () => {
  const response = await $api.lines.getLineNotes(props.line.id.toString())
