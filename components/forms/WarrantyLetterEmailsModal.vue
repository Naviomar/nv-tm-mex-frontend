<template>
  ...
  <v-dialog :model-value="show" @update:model-value="emit('update:show', $event)" max-width="900px">
    <v-card>
      <v-card-title>
        Emails for Warranty Letter #{{ warrantyLetterId }}
      </v-card-title>
      <v-card-text>
        ...
        <v-table v-else-if="emails.length > 0" density="compact">
          ...
          <tbody>
            <tr v-for="(item, index) in emails" :key="`wl-email-${index}`">
              <td>
        ...
        <div v-else class="text-center py-8 text-gray-500">
          No emails associated with this warranty letter
        </div>
      </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
  ...
  <ConsigneeEmailModal
    ...
    :warranty-letter-id="warrantyLetterId"
...
const loadEmails = async () => {
  try {
    loading.value = true
    const response = await $api.consignees.getWarrantyLetterEmails(props.customerId, props.warrantyLetterId)
