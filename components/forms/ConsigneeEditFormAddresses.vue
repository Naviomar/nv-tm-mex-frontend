  ...
  <v-card class="mb-4">
    ...
    <v-card-text>
      ...
      <v-table density="compact">
        ...
        <tbody>
          <tr v-for="(item, index) in addresses" :key="`consg-address-${index}`">
            <td>
...
import { schemaAddress } from '~~/forms/consigneeForm'
...
const emit = defineEmits(['update:consignee'])
...
const getConsigneeAddresses = async () => {
  const response = await $api.consignees.getAddresses(props.consignee.id)
...
const onSuccess = async (values: any) => {
  const body = {
    ...values,
    consignee_id: props.consignee.id,
  }
  const response: any = await $api.consignees.updateAddress(props.consignee.id, body)

  snackbar.add({ type: 'success', text: 'Consignee address created' })
  await getConsigneeAddresses()
  toggle()
}
...
const showConfirmDelete = async (id: any) => {
  ...
  if (result) {
    try {
      ...
      await $api.consignees.deleteAddress(props.consignee.id, id)

      snackbar.add({ type: 'success', text: 'Address deleted' })
      emit('update:consignee')
    } catch (e) {
      console.error(e)
    } finally {
      setTimeout(() => {
