        ...
        <div class="col-span-2">
          ...
          <div v-else class="flex justify-end items-center">
            ...
            <ProcessAuthorizationWrapper
              processName="customer-bank.upsert"
...
const saveDirect = async () => {
  try {
    loadingStore.start()
    await $api.customerBanks.upsert(requestProcessData.value)
...
const showConfirmDelete = async (item: any) => {
  ...
  if (result) {
    try {
      loadingStore.start()
      await $api.customerBanks.delete(item.id)
...
const getData = async () => {
  try {
    loadingStore.start()
    const response = await $api.customerBanks.getCustomerBanks(props.id!.toString())
...
const getCatalogs = async () => {
  try {
    loadingStore.start()
    const response = await $api.customerBanks.formCatalogs()
