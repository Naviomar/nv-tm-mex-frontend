...
const canDirectWrite = computed(() => hasPermission('lines-banks-edit'))
...
const saveDirect = async () => {
  try {
    loadingStore.start()
    await $api.lines.upsertBank({ ...values, line_id: Number(props.id) })
...
const showConfirmDelete = async (item: any) => {
  ...
  if (result) {
    try {
      loadingStore.start()
      await $api.lines.deleteBank(props.id!.toString(), item.id)
...
const getData = async () => {
  try {
    loadingStore.start()
    const response = await $api.lines.getLineBanks(props.id!.toString())
...
const getCatalogs = async () => {
  try {
    loadingStore.start()
    const response = await $api.lines.formBankCatalogs()
