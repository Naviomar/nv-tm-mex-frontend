...
const canDirectWrite = computed(() => hasPermission('suppliers-banks-edit'))
...
const saveDirect = async () => {
  try {
    loadingStore.start()
    await $api.supplierBanks.upsert(requestProcessData.value)
...
const showConfirmDelete = async (item: any) => {
  ...
  if (result) {
    try {
      loadingStore.start()
      await $api.supplierBanks.delete(item.id)
...
const getData = async () => {
  try {
    loadingStore.start()
    const response = await $api.supplierBanks.getSupplierBanks(props.id!)
...
const getCatalogs = async () => {
  try {
    loadingStore.start()
    const response = await $api.supplierBanks.formCatalogs()
