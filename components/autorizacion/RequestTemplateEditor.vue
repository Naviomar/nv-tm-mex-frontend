          ...
          <div class="canvas-scroll">
            ...
            <div class="canvas-section-label mt-2 d-flex align-center justify-space-between">
              ...
              <v-menu>
                ...
                <v-list density="compact">
                  ...
                  <v-list-item prepend-icon="mdi-layers-plus" title="Charge builder (combined buy+sell)" @click="addElement('charge_builder_combined')" />
                  <v-list-item prepend-icon="mdi-layers-plus" title="Charge builder (buy rate)" @click="addElement('charge_builder_buyrate')" />
            ...
            <div class="element-list mt-1" @dragover.prevent>
              ...
              >
                ...
                <v-tooltip v-if="el.type === 'charge_builder' || el.type === 'charge_builder_combined' || el.type === 'charge_builder_buyrate' || el.type === 'invoice_charge_builder' || el.type === 'file_upload'" text="Code-driven — not configurable" location="top">
...
function addElement(type: ITemplateElementType) {
  ...
  if (type === 'alert_block') {
    ...
    return
  } else if (type === 'charge_builder_combined') {
    el = { ...base, type, variant: 'sea-export', charges_catalog_key: 'charges' } as any
    draft.value.elements.push(el)
    return
  } else if (type === 'charge_builder_buyrate') {
    el = { ...base, type, charges_catalog_key: 'charges' } as any
    draft.value.elements.push(el)
    return
...
function openEditDialog(idx: number) {
  ...
  if (!el || el.type === 'charge_builder' || el.type === 'charge_builder_combined' || el.type === 'charge_builder_buyrate' || el.type === 'invoice_charge_builder' || el.type === 'file_upload') return
...
function elementLabel(type: string): string {
  return { form_field: 'Field', section: 'Section', text_block: 'Text', alert_block: 'Alert', charge_builder: 'Charge builder', charge_builder_combined: 'Charge builder (combined)', charge_builder_buyrate: 'Charge builder (buy rate)', invoice_charge_builder: 'Invoice charge builder', file_upload: 'File upload' }[type] ?? type
}

function elementColor(type: string): string {
  return { form_field: 'primary', section: 'purple', text_block: 'grey', alert_block: 'orange', charge_builder: 'teal', charge_builder_combined: 'teal', charge_builder_buyrate: 'teal', invoice_charge_builder: 'cyan', file_upload: 'indigo' }[type] ?? 'grey'
}
...
function elementSummary(el: ITemplateElement): string {
  ...
  if (el.type === 'charge_builder_combined') return `variant: ${(el as any).variant}, catalog: ${(el as any).charges_catalog_key}`
  if (el.type === 'charge_builder_buyrate') return `catalog: ${(el as any).charges_catalog_key}`
