  ...
  <v-card>
    ...
    <v-card-text>
      <v-alert
        ...
        class="mb-4"
      >
        Seleccione un usuario para gestionar sus alertas
      </v-alert>

      <div v-else>
        <div class="d-flex justify-space-between align-center mb-4">
          <p class="text-body-2 text-medium-emphasis">
            Tipos de alerta que recibirá este usuario
          </p>
          <v-btn
            size="small"
            color="primary"
        ...
        <v-alert v-else type="info" variant="tonal">
          Este usuario no tiene alertas asignadas directamente.
          Las alertas pueden estar asignadas por roles.
        </v-alert>
      </div>
    </v-card-text>
    ...
    <v-dialog v-model="dialog" max-width="600">
      <v-card>
        <v-card-title class="bg-primary">
          <span class="text-white">Asignar Tipos de Alerta</span>
        </v-card-title>
...
const fetchUserAlerts = async () => {
  ...
  try {
    ...
    assignedAlerts.value = allTypes.filter((type: IAlertType) => 
      type.users?.some((u: any) => u.id === props.userId)
    )
  } catch (error: any) {
...
const saveAssignments = async () => {
  ...
  saving.value = true
  try {
    // Update each alert type to include/exclude this user
    const promises = availableAlertTypes.value.map(async (alertType) => {
      const currentUsers = alertType.users?.map((u: any) => u.id) || []
      const shouldInclude = selectedAlertTypes.value.includes(alertType.id)
      const isIncluded = currentUsers.includes(props.userId!)

      if (shouldInclude && !isIncluded) {
        // Add user
        await api.alerts.syncTypeUsers(alertType.id, [...currentUsers, props.userId!])
      } else if (!shouldInclude && isIncluded) {
        // Remove user
        await api.alerts.syncTypeUsers(
          alertType.id,
          currentUsers.filter(id => id !== props.userId)
        )
      }
    })
...
const removeAlert = async (alert: IAlertType) => {
  ...
  try {
    const currentUsers = alert.users?.map((u: any) => u.id) || []
    await api.alerts.syncTypeUsers(
      alert.id,
      currentUsers.filter(id => id !== props.userId)
    )
