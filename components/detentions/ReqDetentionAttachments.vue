<template>
  <div>
    ...
    <v-dialog v-model="form.showDialog" max-width="600">
      <v-card>
        <v-card-title>
          <span class="headline">Attachments - Request detention {{ serviceName }}</span>
        </v-card-title>
        <v-card-text>
          <v-table density="compact">
            ...
            <tbody>
              ...
              <tr v-for="(attach, index) in attachments" :key="`proof-${index}`">
                ...
                <td>{{ formatDateString(attach.created_at) }} by {{ attach.creator?.name }}</td>
              </tr>
            </tbody>
          </v-table>
          <div class="flex flex-col gap-2 py-4">
            ...
            <v-btn color="blue-darken-1" @click="onUploadClick">Upload</v-btn>
          </div>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
...
const serviceName = computed(() => props.reqDetention?.folio || '#' + props.reqDetention?.id)
...
const showConfirmDelete = async (item: any) => {
  ...
  if (result) {
    try {
      ...
      await $api.maritimeDetentions.removeAttachment(props.reqDetention.id.toString(), body)
...
const onUploadClick = async () => {
  try {
    ...
    loadingStore.start()
    const response = (await $api.maritimeDetentions.uploadAttachments(props.reqDetention.id, body)) as any
...
watch(
  ...
  async (service) => {
    if (service) {
      attachments.value = service.supports || []
    }
  },
  { immediate: true }
)
